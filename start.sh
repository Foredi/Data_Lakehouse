...
for sub_dir in $dirs; do
    sudo docker-compose -f ./services/$sub_dir/docker-compose.yml up -d
done

# print all container and ips
echo RUNNING CONTAINERS:

sudo docker inspect -f '{{index (split .Name "/") 1}} {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $(sudo docker ps -aq)