...
DAG_ID = "ELT_enrollment_fact_data"
MIGRATION_DAG_ID = "migration_fit_data"
DEPENDENCY_DAG_ID = "etl_instruction_fact_data"
DAG_SCHEDULE = "0 2 1 * *"
...
with DAG(
    ...
    wait_for_dependency = ExternalTaskSensor(
        task_id='wait_for_etl_instruction_fact_data',
        external_dag_id=DEPENDENCY_DAG_ID,
        external_task_id=None,  # Set to None to wait for the DAG run
        allowed_states=['success'],
        failed_states=['failed'],
        mode='poke',  # or 'reschedule' based on your needs
        timeout=600,    # Timeout in seconds
        poke_interval=60,  # How often to poke
        dag=dag,
    )
    ls_raw_tables = [
        "enrollment_fact",
